...
class Students_list_DB
  ...
  # get student by id
  def get_by_id(id)
    result = self.connection.exec("SELECT * FROM student WHERE id = #{id}")
    result = result.first
  ...
  def get_k_n_student_short_list(k, n)
    start_index = (k - 1) * n
    result = self.connection.exec("SELECT * FROM student LIMIT #{n} OFFSET #{start_index}").to_a
  ...
  def add_student(student)
    ...
    SQL
    self.connection.exec_params(query, [surname, first_name, last_name, telegram, email, phone_number, git, birthdate])
  end
  ...
  def replace_student_by_id(id, student)
    ...
    query = <<-SQL
      ...
      WHERE id = $9
    SQL
    self.connection.exec_params(query, [surname, first_name, last_name, telegram, email, phone_number, git, birthdate, id])
  end
  ...
  def get_student_short_count
    result = self.connection.exec("SELECT COUNT(*) FROM student")
    result.first["count"]
  end
